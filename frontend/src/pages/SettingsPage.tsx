...
function CompanyPanel() {
  ...
  const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const formErrors = useFormErrors()
  ...
  const removeLogo = async () => { if (!(await confirm({ message: c('¿Eliminar el logo de la empresa? Dejará de aparecer en los documentos que se impriman a partir de ahora.', 'Remove the company logo? It will no longer appear on documents printed from now on.'), danger: true }))) return; try { await apiFetch('/api/v1/company-settings/current/logo', { method: 'DELETE' }); await load(); notify(c('Logo eliminado.', 'Logo removed.')) } catch (cause) { notify(errorMessage(cause), 'error') } }
  ...
  return <section className="panel settings-panel"><form onSubmit={save}><FormErrors value={formErrors.context}><div className="settings-logo-row"><div className="settings-logo-preview">{logoUrl ? <img src={logoUrl} alt={c('Logo de empresa', 'Company logo')} /> : <Building2 size={28} />}</div><div><strong>{c('Identidad visual', 'Visual identity')}</strong><p>{c('PNG, JPEG o WebP; máximo 2 MiB.', 'PNG, JPEG or WebP; 2 MiB maximum.')}</p><div className="row-actions"><label className="button button-secondary button-small"><Upload size={15} />{c('Subir logo', 'Upload logo')}<input className="visually-hidden" type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => void upload(event.target.files?.[0])} /></label>{value?.logoSha256 && <button className="button button-danger button-small" type="button" onClick={() => void removeLogo()}><Trash2 size={14} />{c('Eliminar', 'Delete')}</button>}</div></div></div>
    <div className="form-grid settings-form-grid"><Field label={c('Nombre visible', 'Display name')} htmlFor="company-display" name="displayName" required><input id="company-display" value={form.displayName} onChange={(event) => change('displayName', event.target.value)} required /></Field><Field label={c('País ISO', 'ISO country')} htmlFor="company-country" name="countryCode" required><input id="company-country" maxLength={2} value={form.countryCode} onChange={(event) => change('countryCode', event.target.value.toUpperCase())} required /></Field><Field label={c('Idioma regional', 'Locale')} htmlFor="company-locale" name="locale" required><input id="company-locale" value={form.locale} onChange={(event) => change('locale', event.target.value)} required /></Field><Field label={c('Zona horaria', 'Time zone')} htmlFor="company-zone" name="timezone" required><input id="company-zone" value={form.timezone} onChange={(event) => change('timezone', event.target.value)} required /></Field><Field label={c('Moneda base', 'Base currency')} htmlFor="company-currency" name="baseCurrency" required><input id="company-currency" maxLength={3} value={form.baseCurrency} onChange={(event) => change('baseCurrency', event.target.value.toUpperCase())} required /></Field><Field label={c('Teléfono', 'Phone')} htmlFor="company-phone" name="phone"><input id="company-phone" value={form.phone ?? ''} onChange={(event) => change('phone', event.target.value)} /></Field><Field label={c('Correo de contacto', 'Contact email')} htmlFor="company-contact" name="contactEmail"><input id="company-contact" type="email" value={form.contactEmail ?? ''} onChange={(event) => change('contactEmail', event.target.value)} /></Field><Field label={c('Correo de facturación', 'Invoice email')} htmlFor="company-invoice" name="invoiceEmail"><input id="company-invoice" type="email" value={form.invoiceEmail ?? ''} onChange={(event) => change('invoiceEmail', event.target.value)} /></Field><Field label={c('Correo de respuesta', 'Reply-to email')} htmlFor="company-reply" name="replyToEmail"><input id="company-reply" type="email" value={form.replyToEmail ?? ''} onChange={(event) => change('replyToEmail', event.target.value)} /></Field><Field label={c('Sitio web', 'Website')} htmlFor="company-web" name="website"><input id="company-web" type="url" value={form.website ?? ''} onChange={(event) => change('website', event.target.value)} /></Field><Field label={c('Dirección', 'Address')} htmlFor="company-address" name="addressLine1" wide><input id="company-address" value={form.addressLine1 ?? ''} onChange={(event) => change('addressLine1', event.target.value)} /></Field><Field label={c('Dirección adicional', 'Additional address')} htmlFor="company-address2" name="addressLine2" wide><input id="company-address2" value={form.addressLine2 ?? ''} onChange={(event) => change('addressLine2', event.target.value)} /></Field><Field label={c('Código postal', 'Postcode')} htmlFor="company-postal" name="postalCode"><input id="company-postal" value={form.postalCode ?? ''} onChange={(event) => change('postalCode', event.target.value)} /></Field><Field label={c('Ciudad', 'City')} htmlFor="company-city" name="city"><input id="company-city" value={form.city ?? ''} onChange={(event) => change('city', event.target.value)} /></Field><Field label={c('Provincia / región', 'Region')} htmlFor="company-region" name="region"><input id="company-region" value={form.region ?? ''} onChange={(event) => change('region', event.target.value)} /></Field></div>{error && <div className="form-error">{error}</div>}<div className="form-actions"><button className="button button-primary" disabled={saving}>{saving ? c('Guardando…', 'Saving…') : c('Guardar parámetros', 'Save settings')}</button></div></FormErrors></form></section>
}
...
function NumberingForm({ value, onCancel, onSaved }: { value: NumberingScheme | null; onCancel: () => void; onSaved: () => void }) {
  const { language, t } = useTranslation(); const c = (es: string, en: string) => language === 'es' ? es : en
  const [form, setForm] = useState<Omit<NumberingScheme, 'id'>>(value ?? { code: '', name: '', documentType: 'INVOICE', series: 'A', pattern: '{yyyy}-{series}-{seq:6}', resetPeriod: 'YEARLY', initialValue: 1, active: true, defaultScheme: false }); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const formErrors = useFormErrors()
  const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); try { await apiFetch(value ? `/api/v1/numbering-schemes/${value.id}` : '/api/v1/numbering-schemes', { method: value ? 'PUT' : 'POST', body: JSON.stringify(form) }); onSaved() } catch (cause) { setError(formErrors.capture(cause)) } finally { setSaving(false) } }
  return <form onSubmit={submit}><FormErrors value={formErrors.context}><div className="form-grid"><Field label={c('Código', 'Code')} htmlFor="number-code" name="code" required><input id="number-code" value={form.code} disabled={Boolean(value)} onChange={(event) => setForm({ ...form, code: event.target.value.toUpperCase() })} required /></Field><Field label={c('Nombre', 'Name')} htmlFor="number-name" name="name" required><input id="number-name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} required /></Field><Field label={c('Tipo documental', 'Document type')} htmlFor="number-type" name="documentType" required><select id="number-type" value={form.documentType} onChange={(event) => setForm({ ...form, documentType: event.target.value as DocumentType })}>{(['QUOTE', 'SALES_ORDER', 'DELIVERY_NOTE', 'INVOICE', 'RECTIFYING_INVOICE', 'WORK_ORDER'] as DocumentType[]).map((type) => <option key={type} value={type}>{t(documentTypeKey[type])}</option>)}</select></Field><Field label={c('Serie', 'Series')} htmlFor="number-series" name="series" required><input id="number-series" value={form.series} onChange={(event) => setForm({ ...form, series: event.target.value.toUpperCase() })} required /></Field><Field label={c('Patrón', 'Pattern')} htmlFor="number-pattern" name="pattern" required wide hint="{yyyy} {yy} {MM} {dd} {series} {seq:6}"><input id="number-pattern" value={form.pattern} onChange={(event) => setForm({ ...form, pattern: event.target.value })} required /></Field><Field label={c('Reinicio', 'Reset period')} htmlFor="number-reset" name="resetPeriod"><select id="number-reset" value={form.resetPeriod} onChange={(event) => setForm({ ...form, resetPeriod: event.target.value as ResetPeriod })}>{(['YEARLY', 'MONTHLY', 'DAILY', 'NEVER'] as ResetPeriod[]).map((period) => <option key={period} value={period}>{resetPeriodLabel(period, language)}</option>)}</select></Field><Field label={c('Valor inicial', 'Initial value')} htmlFor="number-initial" name="initialValue"><input id="number-initial" type="number" min="1" value={form.initialValue} onChange={(event) => setForm({ ...form, initialValue: Number(event.target.value) })} /></Field><Field label={c('Opciones', 'Options')} htmlFor="number-active" name="active"><label className="switch-row"><input id="number-active" type="checkbox" checked={form.active} onChange={(event) => setForm({ ...form, active: event.target.checked })} /><span>{c('Activa', 'Active')}</span></label><label className="switch-row"><input type="checkbox" checked={form.defaultScheme} onChange={(event) => setForm({ ...form, defaultScheme: event.target.checked })} /><span>{c('Predeterminada', 'Default')}</span></label></Field></div>{error && <div className="form-error">{error}</div>}<FormActions onCancel={onCancel} saving={saving} /></FormErrors></form>
}
...
function CurrencyForm({ value, onCancel, onSaved }: { value: CurrencyValue | null; onCancel: () => void; onSaved: () => void }) {
  const { language } = useTranslation(); const c = (es: string, en: string) => language === 'es' ? es : en
  const [form, setForm] = useState<Omit<CurrencyValue, 'id'>>(value ?? { code: '', name: '', symbol: '', decimalPlaces: 2, baseCurrency: false, active: true }); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const formErrors = useFormErrors()
  const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); try { await apiFetch(value ? `/api/v1/currencies/${value.id}` : '/api/v1/currencies', { method: value ? 'PUT' : 'POST', body: JSON.stringify(form) }); onSaved() } catch (cause) { setError(formErrors.capture(cause)) } finally { setSaving(false) } }
  return <form onSubmit={submit}><FormErrors value={formErrors.context}><div className="form-grid"><Field label={c('Código ISO', 'ISO code')} htmlFor="currency-code" name="code" required><input id="currency-code" maxLength={3} disabled={Boolean(value)} value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value.toUpperCase() })} required /></Field><Field label={c('Nombre', 'Name')} htmlFor="currency-name" name="name" required><input id="currency-name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} required /></Field><Field label={c('Símbolo', 'Symbol')} htmlFor="currency-symbol" name="symbol" required><input id="currency-symbol" value={form.symbol} onChange={(event) => setForm({ ...form, symbol: event.target.value })} required /></Field><Field label={c('Decimales', 'Decimal places')} htmlFor="currency-decimals" name="decimalPlaces"><input id="currency-decimals" type="number" min="0" max="6" value={form.decimalPlaces} onChange={(event) => setForm({ ...form, decimalPlaces: Number(event.target.value) })} /></Field><Field label={c('Opciones', 'Options')} htmlFor="currency-active" name="active" wide><label className="switch-row"><input id="currency-active" type="checkbox" checked={form.active} onChange={(event) => setForm({ ...form, active: event.target.checked })} /><span>{c('Activa', 'Active')}</span></label><label className="switch-row"><input type="checkbox" checked={form.baseCurrency} onChange={(event) => setForm({ ...form, baseCurrency: event.target.checked })} /><span>{c('Moneda base', 'Base currency')}</span></label></Field></div>{error && <div className="form-error">{error}</div>}<FormActions onCancel={onCancel} saving={saving} /></FormErrors></form>
}
...
function RateForm({ value, currencies, onCancel, onSaved }: { value: ExchangeRate | null; currencies: CurrencyValue[]; onCancel: () => void; onSaved: () => void }) {
  const { language } = useTranslation(); const c = (es: string, en: string) => language === 'es' ? es : en
  const today = localIsoDate(); const [form, setForm] = useState(value ?? { id: '', baseCode: currencies[0]?.code ?? 'EUR', quoteCode: currencies[1]?.code ?? 'USD', rate: 1, rateDate: today, source: 'MANUAL', active: true }); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const formErrors = useFormErrors()
  const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); try { await apiFetch(value ? `/api/v1/exchange-rates/${value.id}` : '/api/v1/exchange-rates', { method: value ? 'PUT' : 'POST', body: JSON.stringify(value ? { rate: form.rate, active: form.active } : form) }); onSaved() } catch (cause) { setError(formErrors.capture(cause)) } finally { setSaving(false) } }
  return <form onSubmit={submit}><FormErrors value={formErrors.context}><div className="form-grid"><Field label={c('Moneda base', 'Base currency')} htmlFor="rate-base" name="baseCode" required><select id="rate-base" disabled={Boolean(value)} value={form.baseCode} onChange={(event) => setForm({ ...form, baseCode: event.target.value })}>{currencies.map((item) => <option key={item.id}>{item.code}</option>)}</select></Field><Field label={c('Moneda destino', 'Quote currency')} htmlFor="rate-quote" name="quoteCode" required><select id="rate-quote" disabled={Boolean(value)} value={form.quoteCode} onChange={(event) => setForm({ ...form, quoteCode: event.target.value })}>{currencies.map((item) => <option key={item.id}>{item.code}</option>)}</select></Field><Field label={c('Tipo de cambio', 'Exchange rate')} htmlFor="rate-value" name="rate" required><input id="rate-value" type="number" min="0.00000001" step="any" value={form.rate} onChange={(event) => setForm({ ...form, rate: Number(event.target.value) })} required /></Field><Field label={c('Fecha', 'Date')} htmlFor="rate-date" name="rateDate" required><input id="rate-date" type="date" disabled={Boolean(value)} value={form.rateDate} onChange={(event) => setForm({ ...form, rateDate: event.target.value })} required /></Field><Field label={c('Fuente', 'Source')} htmlFor="rate-source" name="source" required><input id="rate-source" disabled={Boolean(value)} value={form.source} onChange={(event) => setForm({ ...form, source: event.target.value })} required /></Field><Field label={c('Estado', 'Status')} htmlFor="rate-active" name="active"><label className="switch-row"><input id="rate-active" type="checkbox" checked={form.active} onChange={(event) => setForm({ ...form, active: event.target.checked })} /><span>{c('Activo', 'Active')}</span></label></Field></div>{error && <div className="form-error">{error}</div>}<FormActions onCancel={onCancel} saving={saving} /></FormErrors></form>
}
...
function LicenseForm({ onCancel, onCreated }: { onCancel: () => void; onCreated: (code: string) => void }) {
  ...
  const [form, setForm] = useState({ displayName: '', validFrom: localIsoDateTime(now), validUntil: localIsoDateTime(until), gracePeriodSeconds: 604800, maxInstallations: 1, checkIntervalSeconds: 3600, features: '' }); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const formErrors = useFormErrors()
  const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); try { const response = await apiFetch<{ activationCode: string }>('/api/v1/licenses', { method: 'POST', body: JSON.stringify({ ...form, validFrom: form.validFrom ? new Date(form.validFrom).toISOString() : null, validUntil: new Date(form.validUntil).toISOString(), features: form.features.split(',').map((value) => value.trim()).filter(Boolean) }) }); onCreated(response.activationCode) } catch (cause) { setError(formErrors.capture(cause)) } finally { setSaving(false) } }
  return <form onSubmit={submit}><FormErrors value={formErrors.context}><div className="form-grid"><Field label={c('Nombre', 'Name')} htmlFor="license-name" name="displayName" required wide><input id="license-name" value={form.displayName} onChange={(event) => setForm({ ...form, displayName: event.target.value })} required /></Field><Field label={c('Válida desde', 'Valid from')} htmlFor="license-from" name="validFrom"><input id="license-from" type="datetime-local" value={form.validFrom} onChange={(event) => setForm({ ...form, validFrom: event.target.value })} /></Field><Field label={c('Válida hasta', 'Valid until')} htmlFor="license-until" name="validUntil"><input id="license-until" type="datetime-local" value={form.validUntil} onChange={(event) => setForm({ ...form, validUntil: event.target.value })} required /></Field><Field label={c('Gracia (segundos)', 'Grace (seconds)')} htmlFor="license-grace" name="gracePeriodSeconds"><input id="license-grace" type="number" min="0" value={form.gracePeriodSeconds} onChange={(event) => setForm({ ...form, gracePeriodSeconds: Number(event.target.value) })} /></Field><Field label={c('Máx. instalaciones', 'Max installations')} htmlFor="license-installs" name="maxInstallations"><input id="license-installs" type="number" min="1" value={form.maxInstallations} onChange={(event) => setForm({ ...form, maxInstallations: Number(event.target.value) })} /></Field><Field label={c('Comprobación (segundos)', 'Check interval (seconds)')} htmlFor="license-check" name="checkIntervalSeconds"><input id="license-check" type="number" min="60" value={form.checkIntervalSeconds} onChange={(event) => setForm({ ...form, checkIntervalSeconds: Number(event.target.value) })} /></Field><Field label={c('Funciones (separadas por comas)', 'Features (comma-separated)')} htmlFor="license-features" name="features" wide><input id="license-features" value={form.features} onChange={(event) => setForm({ ...form, features: event.target.value })} /></Field></div>{error && <div className="form-error">{error}</div>}<FormActions onCancel={onCancel} saving={saving} submitLabel={c('Emitir licencia', 'Issue licence')} /></FormErrors></form>
}
...
function VerifactuPanel() {
  ...
  return <section className="panel settings-panel">
    <form onSubmit={save}><FormErrors value={formErrors.context}>
      ...
      <div className="form-grid settings-form-grid">
        <Field label={c('NIF del obligado', 'Issuer tax ID')} htmlFor="vf-nif" name="issuerTaxId" required>
        ...
        <Field label={c('Razón social', 'Legal name')} htmlFor="vf-name" name="issuerLegalName" required>
        ...
        <Field label={c('Zona horaria', 'Time zone')} htmlFor="vf-zone" name="timeZone" required
        ...
        <Field label={c('Clave de régimen por defecto', 'Default regime key')} htmlFor="vf-regime" name="defaultRegimeKey"
        ...
        <Field label={c('Calificación por defecto', 'Default operation qualification')} htmlFor="vf-qualification" name="defaultOperationQualification"
        ...
        <Field label={c('Estado', 'Status')} htmlFor="vf-enabled" name="enabled" wide>
