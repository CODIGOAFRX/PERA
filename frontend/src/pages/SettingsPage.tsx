...
function CurrencyConverter({ currencies }: { currencies: CurrencyValue[] }) {
  ...
  const [fromCurrency, setFromCurrency] = useState(activeCurrencies.find((currency) => currency.baseCurrency)?.code ?? activeCurrencies[0]?.code ?? 'EUR')
  const [toCurrency, setToCurrency] = useState(activeCurrencies.find((currency) => currency.code !== fromCurrency)?.code ?? activeCurrencies[0]?.code ?? 'USD')
  ...
  const convert = async (event: FormEvent) => {
    event.preventDefault(); setConverting(true); setResult(null)
    try {
  ...
  return <section className="panel settings-wide"><div className="panel-heading settings-heading"><div><strong>{c('Conversor de moneda', 'Currency converter')}</strong><small>{c('Comprueba el cambio histórico que aplicará el ERP.', 'Check the historical exchange rate the ERP will apply.')}</small></div></div><form className="settings-converter" onSubmit={convert}><Field label={c('Importe', 'Amount')} htmlFor="conversion-amount" required><input id="conversion-amount" type="number" min="0" step="any" value={amount} onChange={(event) => setAmount(Number(event.target.value))} required /></Field><Field label={c('Desde', 'From')} htmlFor="conversion-from" required><select id="conversion-from" value={fromCurrency} onChange={(event) => setFromCurrency(event.target.value)}>{activeCurrencies.map((currency) => <option key={currency.id} value={currency.code}>{currency.code}</option>)}</select></Field><Field label={c('A', 'To')} htmlFor="conversion-to" required><select id="conversion-to" value={toCurrency} onChange={(event) => setToCurrency(event.target.value)}>{activeCurrencies.map((currency) => <option key={currency.id} value={currency.code}>{currency.code}</option>)}</select></Field><Field label={c('Fecha', 'Date')} htmlFor="conversion-date" required><input id="conversion-date" type="date" value={date} onChange={(event) => setDate(event.target.value)} required /></Field><button className="button button-primary" type="submit" disabled={converting || fromCurrency === toCurrency}>{converting ? c('Convirtiendo…', 'Converting…') : c('Convertir', 'Convert')}</button></form>{error && <div className="inline-error">{error}</div>}{result && <div className="conversion-result"><strong>{new Intl.NumberFormat(locale, { style: 'currency', currency: result.sourceCurrency }).format(result.sourceAmount)} = {new Intl.NumberFormat(locale, { style: 'currency', currency: result.targetCurrency }).format(result.targetAmount)}</strong><small>{c('Tipo', 'Rate')} {new Intl.NumberFormat(locale, { maximumFractionDigits: 8 }).format(result.exchangeRate)} · {formatDate(result.rateDate, locale)} · {result.rateSource}{result.inverseRate ? ` · ${c('inverso', 'inverse')}` : ''}</small></div>}</section>
}
