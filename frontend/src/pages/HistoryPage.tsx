...
function AlertRuleForm({ rule, onCancel, onSaved }: { rule: AlertRule | null; onCancel: () => void; onSaved: () => void }) {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); setError(''); const payload = { ...form, action: form.action || null, resourceType: form.resourceType || null, conditionField: form.conditionField || null, conditionOperator: form.conditionField ? form.conditionOperator : null, conditionValue: needsValue ? form.conditionValue || null : null }; try { await apiFetch(rule ? `/api/v1/alert-rules/${rule.id}` : '/api/v1/alert-rules', { method: rule ? 'PUT' : 'POST', body: JSON.stringify(payload) }); onSaved() } catch (cause) { setError(formErrors.capture(cause)) } finally { setSaving(false) } }
  return <form onSubmit={submit}><FormErrors value={formErrors.context}><div className="form-grid"><Field label={t('field.code')} htmlFor="alert-code" name="code" required><input id="alert-code" value={form.code} disabled={Boolean(rule)} onChange={(event) => setForm({ ...form, code: event.target.value.toUpperCase() })} required /></Field><Field label={t('field.name')} htmlFor="alert-name" name="name" required><input id="alert-name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} required /></Field><Field label={t('history.eventType')} htmlFor="alert-event" name="eventType" required><input id="alert-event" value={form.eventType} onChange={(event) => setForm({ ...form, eventType: event.target.value })} required /></Field><Field label={t('history.action')} htmlFor="alert-action" name="action"><input id="alert-action" value={form.action ?? ''} onChange={(event) => setForm({ ...form, action: event.target.value })} /></Field><Field label={t('history.resourceType')} htmlFor="alert-resource" name="resourceType"><input id="alert-resource" value={form.resourceType ?? ''} onChange={(event) => setForm({ ...form, resourceType: event.target.value })} /></Field><Field label={t('history.severityLabel')} htmlFor="alert-severity" name="severity" required><select id="alert-severity" value={form.severity} onChange={(event) => setForm({ ...form, severity: event.target.value as AlertSeverity })}><option value="INFO">{t('history.severity.INFO')}</option><option value="WARNING">{t('history.severity.WARNING')}</option><option value="CRITICAL">{t('history.severity.CRITICAL')}</option></select></Field><Field label={t('history.channel')} htmlFor="alert-channel" name="deliveryChannel" required><select id="alert-channel" value={form.deliveryChannel} onChange={(event) => setForm({ ...form, deliveryChannel: event.target.value as 'IN_APP' })}><option value="IN_APP">{t('history.channel.IN_APP')}</option></select></Field><Field label={t('history.conditionField')} htmlFor="alert-field" name="conditionField"><input id="alert-field" placeholder="statusCode" value={form.conditionField ?? ''} onChange={(event) => setForm({ ...form, conditionField: event.target.value, conditionOperator: event.target.value ? form.conditionOperator ?? 'EQUALS' : null })} /></Field><Field label={t('history.operator')} htmlFor="alert-operator" name="conditionOperator"><select id="alert-operator" disabled={!hasCondition} value={form.conditionOperator ?? ''} onChange={(event) => setForm({ ...form, conditionOperator: event.target.value as AlertConditionOperator })}><option value="">—</option>{conditionOperators.map((operator) => <option key={operator} value={operator}>{t(`history.operator.${operator}`)}</option>)}</select></Field><Field label={t('history.conditionValue')} htmlFor="alert-value" name="conditionValue"><input id="alert-value" disabled={!needsValue} value={form.conditionValue ?? ''} onChange={(event) => setForm({ ...form, conditionValue: event.target.value })} /></Field><Field label={t('history.cooldown')} htmlFor="alert-cooldown" name="cooldownMinutes"><input id="alert-cooldown" type="number" min="0" max="525600" value={form.cooldownMinutes} onChange={(event) => setForm({ ...form, cooldownMinutes: Number(event.target.value) })} /></Field><Field label={t('history.titleTemplate')} htmlFor="alert-title" name="titleTemplate" required wide><input id="alert-title" value={form.titleTemplate} onChange={(event) => setForm({ ...form, titleTemplate: event.target.value })} required /></Field><Field label={t('history.messageTemplate')} htmlFor="alert-message" name="messageTemplate" required wide><textarea id="alert-message" rows={3} value={form.messageTemplate} onChange={(event) => setForm({ ...form, messageTemplate: event.target.value })} required /></Field><Field label={t('field.status')} htmlFor="alert-active" name="active"><label className="switch-row" htmlFor="alert-active"><input id="alert-active" type="checkbox" checked={form.active} onChange={(event) => setForm({ ...form, active: event.target.checked })} /><span>{t('history.ruleActive')}</span></label></Field></div>{error && <div className="form-error" role="alert">{error}</div>}<FormActions onCancel={onCancel} saving={saving} /></FormErrors></form>
}
