...
import { localIsoDate, localIsoDateTime } from '../lib/date'
...
export function OperationsPage() {
  ...
  const deleteResource = async (kind: 'template' | 'carrier' | 'vehicle' | 'route' | 'freight' | 'shipment', id: string) => {
    ...
    if (!(await confirm({ message: local(language, 'Esta acción eliminará el registro. ¿Quieres continuar?', 'This action will delete the record. Do you want to continue?'), danger: true }))) return
    try {
  ...
  const runTemplateAction = async (action: 'publish' | 'version' | 'retire', template: WorkflowTemplate) => {
    ...
    if (action === 'retire' && !(await confirm({ message: local(language, 'La versión publicada dejará de estar disponible para nuevas ejecuciones.', 'The published version will no longer be available for new executions.'), danger: true }))) return
    try {
  ...
  const runStepAction = async (execution: WorkExecution, step: WorkStep, action: 'start' | 'complete' | 'skip' | 'cancel') => {
    ...
    if (action === 'cancel' && !(await confirm({ message: local(language, 'Esta acción cancelará la ejecución y todos sus pasos pendientes.', 'This action will cancel the execution and all its pending steps.'), danger: true }))) return
    try {
  ...
  const runShipmentTransition = async (shipment: Shipment, action: ShipmentTransition) => {
    ...
    if (action === 'cancel' && !(await confirm({ message: local(language, '¿Confirmas la cancelación de la expedición?', 'Do you confirm cancelling the shipment?'), danger: true }))) return
    try {
  ...
  const removeShipmentDocument = async (shipment: Shipment, documentId: string) => {
    if (!(await confirm({ message: local(language, '¿Eliminar definitivamente el archivo y sus metadatos?', 'Permanently delete the file and its metadata?'), danger: true }))) return
    try {
  ...
  const downloadShipmentDocument = async (shipment: Shipment, document: ShipmentDocument) => {
    try {
      ...
      saveBlob(download.blob, download.filename || document.originalFileName)
...
function TemplateForm({ item, onCancel, onSaved }: { item?: WorkflowTemplate; onCancel: () => void; onSaved: () => void }) {
  ...
  return <form onSubmit={submit}><FormErrors value={formErrors.context}>
    <div className="form-grid">
      <Field label={local(language, 'Código', 'Code')} htmlFor="workflow-code" name="code" required><input id="workflow-code" value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value })} pattern="[A-Za-z0-9][A-Za-z0-9_-]{0,59}" maxLength={60} disabled={Boolean(item)} required /></Field>
      <Field label={local(language, 'Nombre', 'Name')} htmlFor="workflow-name" name="name" required><input id="workflow-name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} maxLength={180} required /></Field>
      <Field label={local(language, 'Tipo de referencia', 'Reference type')} htmlFor="workflow-reference" name="referenceType" required hint={local(language, 'Ej.: SALES_ORDER, SHIPMENT o SERVICE_REQUEST', 'E.g. SALES_ORDER, SHIPMENT or SERVICE_REQUEST')}><input id="workflow-reference" value={form.referenceType} onChange={(event) => setForm({ ...form, referenceType: event.target.value })} pattern="[A-Za-z0-9][A-Za-z0-9_.-]{0,79}" maxLength={80} required /></Field>
    </div>
    ...
    <div className="line-editor">{steps.map((step, index) => <div className="panel" key={index}>
      <div className="form-grid">
        <Field label={`${local(language, 'Código del paso', 'Step code')} ${index + 1}`} htmlFor={`step-code-${index}`} name={`steps[${index}].code`} required><input id={`step-code-${index}`} value={step.code} onChange={(event) => updateStep(index, 'code', event.target.value)} pattern="[A-Za-z0-9][A-Za-z0-9_-]{0,59}" maxLength={60} required /></Field>
        <Field label={local(language, 'Nombre', 'Name')} htmlFor={`step-name-${index}`} name={`steps[${index}].name`} required><input id={`step-name-${index}`} value={step.name} onChange={(event) => updateStep(index, 'name', event.target.value)} maxLength={180} required /></Field>
        <Field label={local(language, 'Minutos estimados', 'Estimated minutes')} htmlFor={`step-minutes-${index}`} name={`steps[${index}].estimatedMinutes`}><input id={`step-minutes-${index}`} type="number" min="0" step="1" value={step.estimatedMinutes} onChange={(event) => updateStep(index, 'estimatedMinutes', event.target.value)} /></Field>
        <Field label={local(language, 'Obligatorio', 'Required')} htmlFor={`step-required-${index}`} name={`steps[${index}].required`}><label className="switch-row" htmlFor={`step-required-${index}`}><input id={`step-required-${index}`} type="checkbox" checked={step.required} onChange={(event) => updateStep(index, 'required', event.target.checked)} /><span>{step.required ? local(language, 'Sí', 'Yes') : local(language, 'No', 'No')}</span></label></Field>
        <Field label={local(language, 'Descripción', 'Description')} htmlFor={`step-description-${index}`} name={`steps[${index}].description`} wide><textarea id={`step-description-${index}`} rows={2} value={step.description} onChange={(event) => updateStep(index, 'description', event.target.value)} maxLength={2000} /></Field>
      </div>
...
function ExecutionForm({ templates, onCancel, onSaved }: { templates: WorkflowTemplate[]; onCancel: () => void; onSaved: (execution: WorkExecution) => void }) {
  ...
  return <form onSubmit={submit}><FormErrors value={formErrors.context}><div className="form-grid">
    <Field label={local(language, 'Plantilla publicada', 'Published template')} htmlFor="execution-template" name="templateId" required><select id="execution-template" value={templateId} onChange={(event) => setTemplateId(event.target.value)} required>{published.map((item) => <option key={item.id} value={item.id}>{item.code} · v{item.templateVersion} · {item.name}</option>)}</select></Field>
    ...
    <Field label={local(language, 'ID de referencia', 'Reference ID')} htmlFor="execution-reference-id" name="referenceId" required hint={local(language, 'UUID del pedido, expedición, servicio u objeto asociado', 'UUID of the related order, shipment, service or object')}><input id="execution-reference-id" value={referenceId} onChange={(event) => setReferenceId(event.target.value)} pattern="[0-9a-fA-F-]{36}" maxLength={36} required /></Field>
  </div>{error && <div className="form-error" role="alert">{error}</div>}<FormActions onCancel={onCancel} saving={saving} submitLabel={local(language, 'Crear ejecución', 'Create execution')} /></FormErrors></form>
}
...
function CarrierForm({ item, onCancel, onSaved }: { item?: Carrier; onCancel: () => void; onSaved: () => void }) {
  ...
  const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const formErrors = useFormErrors()
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setSaving(true); setError('')
  ...
  return <form onSubmit={submit}><FormErrors value={formErrors.context}><div className="form-grid">
    <Field label={local(language, 'Código', 'Code')} htmlFor="carrier-code" name="code" required><input id="carrier-code" value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value })} pattern="[A-Za-z0-9][A-Za-z0-9_-]{0,59}" maxLength={60} disabled={Boolean(item)} required /></Field>
    <Field label={local(language, 'Nombre', 'Name')} htmlFor="carrier-name" name="name" required><input id="carrier-name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} maxLength={180} required /></Field>
    <Field label={local(language, 'Titularidad', 'Ownership')} htmlFor="carrier-ownership" name="ownership" required><select id="carrier-ownership" value={form.ownership} onChange={(event) => setForm({ ...form, ownership: event.target.value as CarrierOwnership })}><option value="OWN">{local(language, 'Flota propia', 'Own fleet')}</option><option value="THIRD_PARTY">{local(language, 'Tercero', 'Third party')}</option></select></Field>
    <Field label={local(language, 'NIF / identificación fiscal', 'Tax identifier')} htmlFor="carrier-tax" name="taxIdentifier"><input id="carrier-tax" value={form.taxIdentifier} onChange={(event) => setForm({ ...form, taxIdentifier: event.target.value })} maxLength={40} /></Field>
    <Field label={local(language, 'Identificador externo', 'External identifier')} htmlFor="carrier-external" name="externalIdentifier"><input id="carrier-external" value={form.externalIdentifier} onChange={(event) => setForm({ ...form, externalIdentifier: event.target.value })} maxLength={100} /></Field>
    <Field label={local(language, 'Persona de contacto', 'Contact person')} htmlFor="carrier-contact" name="contactName"><input id="carrier-contact" value={form.contactName} onChange={(event) => setForm({ ...form, contactName: event.target.value })} maxLength={180} /></Field>
    <Field label={local(language, 'Correo', 'Email')} htmlFor="carrier-email" name="contactEmail"><input id="carrier-email" type="email" value={form.contactEmail} onChange={(event) => setForm({ ...form, contactEmail: event.target.value })} maxLength={254} /></Field>
    <ContactDetailsFields value={form.details} onChange={details => setForm({ ...form, details })} prefix="carrier" />
    <Field label={local(language, 'Teléfono', 'Phone')} htmlFor="carrier-phone" name="contactPhone"><input id="carrier-phone" value={form.contactPhone} onChange={(event) => setForm({ ...form, contactPhone: event.target.value })} maxLength={40} /></Field>
    <Field label={local(language, 'Estado', 'Status')} htmlFor="carrier-active" name="active"><label className="switch-row" htmlFor="carrier-active"><input id="carrier-active" type="checkbox" checked={form.active} onChange={(event) => setForm({ ...form, active: event.target.checked })} /><span>{form.active ? local(language, 'Activo', 'Active') : local(language, 'Inactivo', 'Inactive')}</span></label></Field>
  </div>{error && <div className="form-error" role="alert">{error}</div>}<FormActions onCancel={onCancel} saving={saving} submitLabel={item ? local(language, 'Guardar transportista', 'Save carrier') : local(language, 'Crear transportista', 'Create carrier')} /></FormErrors></form>
}
...
function VehicleForm({ item, carriers, onCancel, onSaved }: { item?: Vehicle; carriers: Carrier[]; onCancel: () => void; onSaved: () => void }) {
  ...
  return <form onSubmit={submit}><FormErrors value={formErrors.context}><div className="form-grid">
    <Field label={local(language, 'Código', 'Code')} htmlFor="vehicle-code" name="code" required><input id="vehicle-code" value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value })} pattern="[A-Za-z0-9][A-Za-z0-9_-]{0,59}" maxLength={60} disabled={Boolean(item)} required /></Field>
    <Field label={local(language, 'Matrícula', 'Registration plate')} htmlFor="vehicle-plate" name="registrationPlate"><input id="vehicle-plate" value={form.registrationPlate} onChange={(event) => setForm({ ...form, registrationPlate: event.target.value })} maxLength={30} /></Field>
    <Field label={local(language, 'Tipo de vehículo', 'Vehicle type')} htmlFor="vehicle-type" name="vehicleType" required><input id="vehicle-type" value={form.vehicleType} onChange={(event) => setForm({ ...form, vehicleType: event.target.value })} maxLength={80} placeholder={local(language, 'Furgoneta, camión, turismo…', 'Van, truck, car…')} required /></Field>
    <Field label={local(language, 'Transportista', 'Carrier')} htmlFor="vehicle-carrier" name="carrierId"><select id="vehicle-carrier" value={form.carrierId} onChange={(event) => setForm({ ...form, carrierId: event.target.value })}><option value="">{local(language, 'Sin asignar', 'Unassigned')}</option>{carriers.map((carrier) => <option key={carrier.id} value={carrier.id}>{carrier.code} · {carrier.name}</option>)}</select></Field>
    <Field label={local(language, 'Capacidad de peso (kg)', 'Weight capacity (kg)')} htmlFor="vehicle-weight" name="capacityWeightKg"><input id="vehicle-weight" type="number" min="0" step="0.001" value={form.capacityWeightKg} onChange={(event) => setForm({ ...form, capacityWeightKg: event.target.value })} /></Field>
    <Field label={local(language, 'Capacidad de volumen (m³)', 'Volume capacity (m³)')} htmlFor="vehicle-volume" name="capacityVolumeM3"><input id="vehicle-volume" type="number" min="0" step="0.000001" value={form.capacityVolumeM3} onChange={(event) => setForm({ ...form, capacityVolumeM3: event.target.value })} /></Field>
    <Field label={local(language, 'Estado', 'Status')} htmlFor="vehicle-active" name="active"><label className="switch-row" htmlFor="vehicle-active"><input id="vehicle-active" type="checkbox" checked={form.active} onChange={(event) => setForm({ ...form, active: event.target.checked })} /><span>{form.active ? local(language, 'Activo', 'Active') : local(language, 'Inactivo', 'Inactive')}</span></label></Field>
  </div>{error && <div className="form-error" role="alert">{error}</div>}<FormActions onCancel={onCancel} saving={saving} submitLabel={item ? local(language, 'Guardar vehículo', 'Save vehicle') : local(language, 'Crear vehículo', 'Create vehicle')} /></FormErrors></form>
}
...
function RouteForm({ item, carriers, vehicles, onCancel, onSaved }: { item?: DeliveryRoute; carriers: Carrier[]; vehicles: Vehicle[]; onCancel: () => void; onSaved: () => void }) {
  ...
  return <form onSubmit={submit}><FormErrors value={formErrors.context}>
    <div className="form-grid">
      <Field label={local(language, 'Código', 'Code')} htmlFor="route-code" name="code" required><input id="route-code" value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value })} pattern="[A-Za-z0-9][A-Za-z0-9_-]{0,59}" maxLength={60} disabled={Boolean(item)} required /></Field>
      <Field label={local(language, 'Nombre', 'Name')} htmlFor="route-name" name="name" required><input id="route-name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} maxLength={180} required /></Field>
      <Field label={local(language, 'Origen', 'Origin')} htmlFor="route-origin" name="origin" required><input id="route-origin" value={form.origin} onChange={(event) => setForm({ ...form, origin: event.target.value })} maxLength={500} required /></Field>
      <Field label={local(language, 'Destino', 'Destination')} htmlFor="route-destination" name="destination" required><input id="route-destination" value={form.destination} onChange={(event) => setForm({ ...form, destination: event.target.value })} maxLength={500} required /></Field>
      <Field label={local(language, 'Transportista', 'Carrier')} htmlFor="route-carrier" name="carrierId"><select id="route-carrier" value={form.carrierId} onChange={(event) => chooseCarrier(event.target.value)}><option value="">{local(language, 'Sin asignar', 'Unassigned')}</option>{carriers.filter((carrier) => carrier.active || carrier.id === form.carrierId).map((carrier) => <option key={carrier.id} value={carrier.id}>{carrier.code} · {carrier.name}</option>)}</select></Field>
      <Field label={local(language, 'Vehículo', 'Vehicle')} htmlFor="route-vehicle" name="vehicleId"><select id="route-vehicle" value={form.vehicleId} onChange={(event) => setForm({ ...form, vehicleId: event.target.value })}><option value="">{local(language, 'Sin asignar', 'Unassigned')}</option>{matchingVehicles.filter((vehicle) => vehicle.active || vehicle.id === form.vehicleId).map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.code} · {vehicle.registrationPlate ?? vehicle.vehicleType}</option>)}</select></Field>
      <Field label={local(language, 'Salida prevista', 'Planned departure')} htmlFor="route-departure" name="plannedDepartureAt"><input id="route-departure" type="datetime-local" value={form.plannedDepartureAt} onChange={(event) => setForm({ ...form, plannedDepartureAt: event.target.value })} /></Field>
      <Field label={local(language, 'Llegada prevista', 'Planned arrival')} htmlFor="route-arrival" name="plannedArrivalAt"><input id="route-arrival" type="datetime-local" value={form.plannedArrivalAt} onChange={(event) => setForm({ ...form, plannedArrivalAt: event.target.value })} /></Field>
      <Field label={local(language, 'Inicio ventana de entrega', 'Delivery window start')} htmlFor="route-window-start" name="deliveryWindowStart"><input id="route-window-start" type="datetime-local" value={form.deliveryWindowStart} onChange={(event) => setForm({ ...form, deliveryWindowStart: event.target.value })} /></Field>
      <Field label={local(language, 'Fin ventana de entrega', 'Delivery window end')} htmlFor="route-window-end" name="deliveryWindowEnd"><input id="route-window-end" type="datetime-local" value={form.deliveryWindowEnd} onChange={(event) => setForm({ ...form, deliveryWindowEnd: event.target.value })} /></Field>
      <Field label={local(language, 'Distancia (km)', 'Distance (km)')} htmlFor="route-distance" name="distanceKm" hint={local(language, 'Se usa automáticamente al resolver fletes por kilómetro.', 'Used automatically when resolving per-kilometre freight.')}><input id="route-distance" type="number" min="0.001" step="0.001" value={form.distanceKm} onChange={(event) => setForm({ ...form, distanceKm: event.target.value })} /></Field>
      <Field label={local(language, 'Duración estimada (min)', 'Estimated duration (min)')} htmlFor="route-duration" name="estimatedDurationMinutes"><input id="route-duration" type="number" min="1" step="1" value={form.estimatedDurationMinutes} onChange={(event) => setForm({ ...form, estimatedDurationMinutes: event.target.value })} /></Field>
      <Field label={local(language, 'Estado', 'Status')} htmlFor="route-active" name="active"><label className="switch-row" htmlFor="route-active"><input id="route-active" type="checkbox" checked={form.active} onChange={(event) => setForm({ ...form, active: event.target.checked })} /><span>{form.active ? local(language, 'Activa', 'Active') : local(language, 'Inactiva', 'Inactive')}</span></label></Field>
    </div>
    ...
    {stops.length === 0 ? <EmptyState title={local(language, 'Ruta directa', 'Direct route')} description={local(language, 'No hay paradas intermedias configuradas.', 'No intermediate stops are configured.')} /> : <div className="line-editor">{stops.map((stop, index) => <div className="panel" key={index}>
      <div className="form-grid">
        <Field label={`${local(language, 'Parada', 'Stop')} ${index + 1}`} htmlFor={`stop-name-${index}`} name={`stops[${index}].name`} required><input id={`stop-name-${index}`} value={stop.name} onChange={(event) => updateStop(index, 'name', event.target.value)} maxLength={180} required /></Field>
        <Field label={local(language, 'Ubicación', 'Location')} htmlFor={`stop-location-${index}`} name={`stops[${index}].location`} required><input id={`stop-location-${index}`} value={stop.location} onChange={(event) => updateStop(index, 'location', event.target.value)} maxLength={500} required /></Field>
        <Field label={local(language, 'Inicio de ventana', 'Window start')} htmlFor={`stop-start-${index}`} name={`stops[${index}].windowStart`}><input id={`stop-start-${index}`} type="datetime-local" value={stop.windowStart} onChange={(event) => updateStop(index, 'windowStart', event.target.value)} /></Field>
        <Field label={local(language, 'Fin de ventana', 'Window end')} htmlFor={`stop-end-${index}`} name={`stops[${index}].windowEnd`}><input id={`stop-end-${index}`} type="datetime-local" value={stop.windowEnd} onChange={(event) => updateStop(index, 'windowEnd', event.target.value)} /></Field>
        <Field label={local(language, 'Instrucciones', 'Instructions')} htmlFor={`stop-instructions-${index}`} name={`stops[${index}].instructions`} wide><textarea id={`stop-instructions-${index}`} rows={2} value={stop.instructions} onChange={(event) => updateStop(index, 'instructions', event.target.value)} maxLength={1000} /></Field>
...
function FreightRateForm({ item, carriers, routes, onCancel, onSaved }: { item?: FreightRate; carriers: Carrier[]; routes: DeliveryRoute[]; onCancel: () => void; onSaved: () => void }) {
  ...
  return <form onSubmit={submit}><FormErrors value={formErrors.context}>
    <div className="form-grid">
      <Field label={local(language, 'Código', 'Code')} htmlFor="freight-code" name="code" required><input id="freight-code" value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value })} pattern="[A-Za-z0-9][A-Za-z0-9_-]{0,59}" maxLength={60} disabled={Boolean(item)} required /></Field>
      <Field label={local(language, 'Nombre', 'Name')} htmlFor="freight-name" name="name" required><input id="freight-name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} maxLength={180} required /></Field>
      <Field label={local(language, 'Ruta aplicable', 'Applicable route')} htmlFor="freight-route" name="routeId"><select id="freight-route" value={form.routeId} onChange={(event) => chooseRoute(event.target.value)}><option value="">{local(language, 'Cualquier ruta', 'Any route')}</option>{routes.map((route) => <option key={route.id} value={route.id}>{route.code} · {route.name}</option>)}</select></Field>
      <Field label={local(language, 'Transportista aplicable', 'Applicable carrier')} htmlFor="freight-carrier" name="carrierId"><select id="freight-carrier" value={form.carrierId} onChange={(event) => setForm({ ...form, carrierId: event.target.value })}><option value="">{local(language, 'Cualquier transportista', 'Any carrier')}</option>{availableCarriers.map((carrier) => <option key={carrier.id} value={carrier.id}>{carrier.code} · {carrier.name}</option>)}</select></Field>
      <Field label={local(language, 'Moneda ISO', 'ISO currency')} htmlFor="freight-currency" name="currencyCode" required><input id="freight-currency" value={form.currencyCode} onChange={(event) => setForm({ ...form, currencyCode: event.target.value })} pattern="[A-Za-z]{3}" minLength={3} maxLength={3} required /></Field>
      <Field label={local(language, 'Prioridad', 'Priority')} htmlFor="freight-priority" name="priority" required hint={local(language, 'La prioridad mayor gana entre tarifas elegibles.', 'The highest priority wins among eligible rates.')}><input id="freight-priority" type="number" min="-1000000" max="1000000" step="1" value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value })} required /></Field>
      <Field label={local(language, 'Válida desde', 'Valid from')} htmlFor="freight-valid-from" name="validFrom" required><input id="freight-valid-from" type="date" value={form.validFrom} onChange={(event) => setForm({ ...form, validFrom: event.target.value })} required /></Field>
      <Field label={local(language, 'Válida hasta', 'Valid to')} htmlFor="freight-valid-to" name="validTo"><input id="freight-valid-to" type="date" value={form.validTo} onChange={(event) => setForm({ ...form, validTo: event.target.value })} /></Field>
      <Field label={local(language, 'Método de cálculo', 'Calculation method')} htmlFor="freight-method" name="calculationMethod" required><select id="freight-method" value={form.calculationMethod} onChange={(event) => setForm({ ...form, calculationMethod: event.target.value as FreightCalculationMethod })}>{freightMethods.map((method) => <option key={method} value={method}>{freightMethodLabel(method, language)}</option>)}</select></Field>
      <Field label={local(language, 'Estado', 'Status')} htmlFor="freight-active" name="active"><label className="switch-row" htmlFor="freight-active"><input id="freight-active" type="checkbox" checked={form.active} onChange={(event) => setForm({ ...form, active: event.target.checked })} /><span>{form.active ? local(language, 'Activa', 'Active') : local(language, 'Inactiva', 'Inactive')}</span></label></Field>
      {requiresFixed && <Field label={local(language, 'Importe fijo', 'Fixed amount')} htmlFor="freight-fixed" name="fixedAmount" required><input id="freight-fixed" type="number" min="0" step="0.0001" value={form.fixedAmount} onChange={(event) => setForm({ ...form, fixedAmount: event.target.value })} required /></Field>}
      {requiresUnit && <Field label={`${local(language, 'Importe por', 'Amount per')} ${freightMetricLabel(form.calculationMethod, language)}`} htmlFor="freight-unit" name="unitAmount" required><input id="freight-unit" type="number" min="0" step="0.000001" value={form.unitAmount} onChange={(event) => setForm({ ...form, unitAmount: event.target.value })} required /></Field>}
      <Field label={local(language, 'Cargo mínimo', 'Minimum charge')} htmlFor="freight-min-charge" name="minimumCharge"><input id="freight-min-charge" type="number" min="0" step="0.0001" value={form.minimumCharge} onChange={(event) => setForm({ ...form, minimumCharge: event.target.value })} /></Field>
      <Field label={local(language, 'Cargo máximo', 'Maximum charge')} htmlFor="freight-max-charge" name="maximumCharge"><input id="freight-max-charge" type="number" min="0" step="0.0001" value={form.maximumCharge} onChange={(event) => setForm({ ...form, maximumCharge: event.target.value })} /></Field>
    </div>
    ...
    <div className="form-grid">
      <Field label={local(language, 'Peso mínimo (kg)', 'Minimum weight (kg)')} htmlFor="freight-min-weight" name="minimumWeightKg"><input id="freight-min-weight" type="number" min="0" step="0.001" value={form.minimumWeightKg} onChange={(event) => setForm({ ...form, minimumWeightKg: event.target.value })} /></Field>
      <Field label={local(language, 'Peso máximo (kg)', 'Maximum weight (kg)')} htmlFor="freight-max-weight" name="maximumWeightKg"><input id="freight-max-weight" type="number" min="0" step="0.001" value={form.maximumWeightKg} onChange={(event) => setForm({ ...form, maximumWeightKg: event.target.value })} /></Field>
      <Field label={local(language, 'Volumen mínimo (m³)', 'Minimum volume (m³)')} htmlFor="freight-min-volume" name="minimumVolumeM3"><input id="freight-min-volume" type="number" min="0" step="0.000001" value={form.minimumVolumeM3} onChange={(event) => setForm({ ...form, minimumVolumeM3: event.target.value })} /></Field>
      <Field label={local(language, 'Volumen máximo (m³)', 'Maximum volume (m³)')} htmlFor="freight-max-volume" name="maximumVolumeM3"><input id="freight-max-volume" type="number" min="0" step="0.000001" value={form.maximumVolumeM3} onChange={(event) => setForm({ ...form, maximumVolumeM3: event.target.value })} /></Field>
      <Field label={local(language, 'Distancia mínima (km)', 'Minimum distance (km)')} htmlFor="freight-min-distance" name="minimumDistanceKm"><input id="freight-min-distance" type="number" min="0" step="0.001" value={form.minimumDistanceKm} onChange={(event) => setForm({ ...form, minimumDistanceKm: event.target.value })} /></Field>
      <Field label={local(language, 'Distancia máxima (km)', 'Maximum distance (km)')} htmlFor="freight-max-distance" name="maximumDistanceKm"><input id="freight-max-distance" type="number" min="0" step="0.001" value={form.maximumDistanceKm} onChange={(event) => setForm({ ...form, maximumDistanceKm: event.target.value })} /></Field>
    </div>
...
function ShipmentFreightForm({ shipment, routes, locale, onCancel, onSaved }: { shipment: Shipment; routes: DeliveryRoute[]; locale: string; onCancel: () => void; onSaved: (shipment: Shipment) => void }) {
  ...
  return <form onSubmit={submit}><FormErrors value={formErrors.context}>
    ...
    <div className="form-grid">
      <Field label={local(language, 'Fecha de tarificación', 'Pricing date')} htmlFor="shipment-freight-date" name="pricingDate" required><input id="shipment-freight-date" type="date" value={pricingDate} onChange={(event) => setPricingDate(event.target.value)} required /></Field>
      <Field label={local(language, 'Distancia (km)', 'Distance (km)')} htmlFor="shipment-freight-distance" name="distanceKm" hint={local(language, 'Opcional: si se deja vacía, se usa la distancia configurada en la ruta. Para tarifas por kilómetro debe existir uno de los dos valores.', 'Optional: when left empty, the configured route distance is used. Per-kilometre rates require one of the two values.')}><input id="shipment-freight-distance" type="number" min="0.001" step="0.001" value={distanceKm} onChange={(event) => setDistanceKm(event.target.value)} /></Field>
    </div>
...
function ShipmentForm({ item, carriers, vehicles, routes, onCancel, onSaved }: { item?: Shipment; carriers: Carrier[]; vehicles: Vehicle[]; routes: DeliveryRoute[]; onCancel: () => void; onSaved: () => void }) {
  ...
  return <form onSubmit={submit}><FormErrors value={formErrors.context}>
    <div className="form-grid">
      <Field label={local(language, 'Número de expedición', 'Shipment number')} htmlFor="shipment-number" name="shipmentNumber" required><input id="shipment-number" value={form.shipmentNumber} onChange={(event) => setForm({ ...form, shipmentNumber: event.target.value })} pattern="[A-Za-z0-9][A-Za-z0-9_./-]{0,79}" maxLength={80} disabled={Boolean(item)} required /></Field>
      <Field label={local(language, 'Ruta', 'Route')} htmlFor="shipment-route" name="routeId"><select id="shipment-route" value={form.routeId} onChange={(event) => chooseRoute(event.target.value)}><option value="">{local(language, 'Sin ruta predefinida', 'No predefined route')}</option>{routes.filter((route) => route.active || route.id === form.routeId).map((route) => <option key={route.id} value={route.id}>{route.code} · {route.name}</option>)}</select></Field>
      <Field label={local(language, 'Origen', 'Origin')} htmlFor="shipment-origin" name="origin"><input id="shipment-origin" value={form.origin} onChange={(event) => setForm({ ...form, origin: event.target.value })} maxLength={500} /></Field>
      <Field label={local(language, 'Destino', 'Destination')} htmlFor="shipment-destination" name="destination"><input id="shipment-destination" value={form.destination} onChange={(event) => setForm({ ...form, destination: event.target.value })} maxLength={500} /></Field>
      <Field label={local(language, 'Transportista', 'Carrier')} htmlFor="shipment-carrier" name="carrierId"><select id="shipment-carrier" value={form.carrierId} onChange={(event) => chooseCarrier(event.target.value)}><option value="">{local(language, 'Sin asignar', 'Unassigned')}</option>{carriers.filter((carrier) => carrier.active || carrier.id === form.carrierId).map((carrier) => <option key={carrier.id} value={carrier.id}>{carrier.code} · {carrier.name}</option>)}</select></Field>
      <Field label={local(language, 'Vehículo', 'Vehicle')} htmlFor="shipment-vehicle" name="vehicleId"><select id="shipment-vehicle" value={form.vehicleId} onChange={(event) => setForm({ ...form, vehicleId: event.target.value })}><option value="">{local(language, 'Sin asignar', 'Unassigned')}</option>{matchingVehicles.filter((vehicle) => vehicle.active || vehicle.id === form.vehicleId).map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.code} · {vehicle.registrationPlate ?? vehicle.vehicleType}</option>)}</select></Field>
      <Field label={local(language, 'Salida prevista', 'Planned departure')} htmlFor="shipment-departure" name="plannedDepartureAt"><input id="shipment-departure" type="datetime-local" value={form.plannedDepartureAt} onChange={(event) => setForm({ ...form, plannedDepartureAt: event.target.value })} /></Field>
      <Field label={local(language, 'Llegada prevista', 'Planned arrival')} htmlFor="shipment-arrival" name="plannedArrivalAt"><input id="shipment-arrival" type="datetime-local" value={form.plannedArrivalAt} onChange={(event) => setForm({ ...form, plannedArrivalAt: event.target.value })} /></Field>
      <Field label={local(language, 'Coste de transporte', 'Freight cost')} htmlFor="shipment-cost" name="freightCost" required><input id="shipment-cost" type="number" min="0" step="0.0001" value={form.freightCost} onChange={(event) => setForm({ ...form, freightCost: event.target.value })} required /></Field>
      <Field label={local(language, 'Moneda ISO', 'ISO currency')} htmlFor="shipment-currency" name="currencyCode" required><input id="shipment-currency" value={form.currencyCode} onChange={(event) => setForm({ ...form, currencyCode: event.target.value })} pattern="[A-Za-z]{3}" minLength={3} maxLength={3} required /></Field>
      <Field label={local(language, 'Peso total (kg)', 'Total weight (kg)')} htmlFor="shipment-weight" name="totalWeightKg"><input id="shipment-weight" type="number" min="0" step="0.001" value={form.totalWeightKg} onChange={(event) => setForm({ ...form, totalWeightKg: event.target.value })} /></Field>
      <Field label={local(language, 'Volumen total (m³)', 'Total volume (m³)')} htmlFor="shipment-volume" name="totalVolumeM3"><input id="shipment-volume" type="number" min="0" step="0.000001" value={form.totalVolumeM3} onChange={(event) => setForm({ ...form, totalVolumeM3: event.target.value })} /></Field>
    </div>
    ...
    <div className="line-editor">{lines.map((line, index) => <div className="panel" key={index}>
      <div className="form-grid">
        <Field label={`${local(language, 'Descripción del item', 'Item description')} ${index + 1}`} htmlFor={`shipment-line-name-${index}`} name={`lines[${index}].productNameSnapshot`} required><input id={`shipment-line-name-${index}`} value={line.productNameSnapshot} onChange={(event) => updateLine(index, 'productNameSnapshot', event.target.value)} maxLength={300} required /></Field>
        <Field label={local(language, 'Cantidad', 'Quantity')} htmlFor={`shipment-line-quantity-${index}`} name={`lines[${index}].quantity`} required><input id={`shipment-line-quantity-${index}`} type="number" min="0.000001" step="0.000001" value={line.quantity} onChange={(event) => updateLine(index, 'quantity', event.target.value)} required /></Field>
        <Field label={local(language, 'Unidad', 'Unit')} htmlFor={`shipment-line-unit-${index}`} name={`lines[${index}].unitOfMeasureSnapshot`} required><input id={`shipment-line-unit-${index}`} value={line.unitOfMeasureSnapshot} onChange={(event) => updateLine(index, 'unitOfMeasureSnapshot', event.target.value)} maxLength={30} placeholder="UNIT, KG, BOX…" required /></Field>
        <Field label={local(language, 'ID de producto', 'Product ID')} htmlFor={`shipment-line-product-id-${index}`} name={`lines[${index}].productId`}><input id={`shipment-line-product-id-${index}`} value={line.productId} onChange={(event) => updateLine(index, 'productId', event.target.value)} maxLength={36} /></Field>
        <Field label={local(language, 'Código snapshot', 'Code snapshot')} htmlFor={`shipment-line-product-code-${index}`} name={`lines[${index}].productCodeSnapshot`}><input id={`shipment-line-product-code-${index}`} value={line.productCodeSnapshot} onChange={(event) => updateLine(index, 'productCodeSnapshot', event.target.value)} maxLength={100} /></Field>
        <Field label={local(language, 'ID de documento origen', 'Source document ID')} htmlFor={`shipment-line-document-id-${index}`} name={`lines[${index}].sourceDocumentId`}><input id={`shipment-line-document-id-${index}`} value={line.sourceDocumentId} onChange={(event) => updateLine(index, 'sourceDocumentId', event.target.value)} maxLength={36} /></Field>
        <Field label={local(language, 'Tipo de documento', 'Document type')} htmlFor={`shipment-line-document-type-${index}`} name={`lines[${index}].sourceDocumentType`}><input id={`shipment-line-document-type-${index}`} value={line.sourceDocumentType} onChange={(event) => updateLine(index, 'sourceDocumentType', event.target.value)} maxLength={80} placeholder="SALES_ORDER" /></Field>
        <Field label={local(language, 'Número snapshot', 'Number snapshot')} htmlFor={`shipment-line-document-number-${index}`} name={`lines[${index}].sourceDocumentNumberSnapshot`}><input id={`shipment-line-document-number-${index}`} value={line.sourceDocumentNumberSnapshot} onChange={(event) => updateLine(index, 'sourceDocumentNumberSnapshot', event.target.value)} maxLength={100} /></Field>
...
function ExecutionDetail({ execution, locale, onAction }: { execution: WorkExecution; locale: string; onAction: (step: WorkStep, action: 'start' | 'complete' | 'skip' | 'cancel') => void }) {
  ...
  return <div className="document-detail">
    ...
    <div className="table-scroll detail-lines"><table><TableCaption es="Pasos de la ejecución" en="Execution steps" /><thead><tr><th>#</th><th>{local(language, 'Paso', 'Step')}</th><th>{local(language, 'Estado', 'Status')}</th><th>{local(language, 'Seguimiento', 'Tracking')}</th><th>{local(language, 'Acciones', 'Actions')}</th></tr></thead><tbody>{execution.steps.map((step, index) => {
